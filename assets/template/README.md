...
5. 读取所有待处理工时单，不再依赖工时单标题中的日期范围。
6. 按顺序进入每张工时单的 `任务工时`，点击 `新增` 打开弹窗。
7. 从弹窗里的 `总工时` 除以每天工时，计算该工时单可承载的天数，并扣除已有工时记录。
8. 用一个待填日期队列保存剩余日期，每保存成功或发现已有记录，就从队列移除对应日期。
9. 在弹窗中再次点击 `新增`，选择工时日期；在配置的工时类型中填写小时数，填写工作说明。
10. 当前工时单容量用完或待填日期填完后切换下一张工时单；当前工时单最后一条工时保存后自动提交。
...
- 每次任务只启动一次浏览器自动化；任务完成或失败后，本地服务会自动关闭并释放端口。
